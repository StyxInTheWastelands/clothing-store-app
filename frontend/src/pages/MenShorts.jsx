...
function MenShorts() {
  usePageMeta('Spodenki męskie', 'Spodenki z kolekcji męskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      ...
      .then((data) => {
        ...
        // Filtrowanie: płeć 'men' oraz kategoria 3 (spodenki)
        const filteredProducts = allProducts.filter(product => {
            ...
            const isShortsCategory = Number(categoryIdField) === 3;

            return isMen && isShortsCategory;
            });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania spodenek męskich:", err))
  ...
  if (loading) {
    return (
      <div style={{ ...backgroundContainerStyle, justifyContent: 'center' }}>
        <h2 style={{ color: '#fff', fontSize: '20px' }}>Ładowanie spodenek...</h2>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych spodenek męskich.</p>
            </div>
          )}
        </div>
      </div>
