...
function WomenSweaters() {
  usePageMeta('Bluzy i swetry damskie', 'Bluzy i swetry z kolekcji damskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      ...
      .then((data) => {
        ...
        // Filtrowanie: płeć żeńska oraz category_id === 5 (bluzy i swetry)
        const filteredProducts = allProducts.filter(product => {
          ...
          const isWomen =
            genderValue.startsWith('w') || 
            genderValue.startsWith('k') ||
            genderValue.includes('kob') ||
            genderValue.includes('wom');

          const categoryIdField = product.category_id !== undefined ? product.category_id : product.categoryId;
          const isSweaterCategory = Number(categoryIdField) === 5;

          return isWomen && isSweaterCategory;
        });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania bluz i swetrów damskich:", err))
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych bluz ani swetrów damskich.</p>
            </div>
          )}
        </div>
      </div>
