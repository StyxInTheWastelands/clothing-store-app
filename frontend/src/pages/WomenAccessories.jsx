...
function WomenAccessories() {
  usePageMeta('Akcesoria damskie', 'Akcesoria z kolekcji damskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      ...
      .then((data) => {
        ...
        // Filtrowanie: płeć żeńska + category_id === 6 (akcesoria)
        const filteredProducts = allProducts.filter(product => {
          ...
          const isAccessoryCategory = Number(categoryIdField) === 6;

          return isWomen && isAccessoryCategory;
        });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania akcesoriów damskich:", err))
  ...
  if (loading) {
    return (
      <div style={{ ...backgroundContainerStyle, justifyContent: 'center' }}>
        <h2 style={{ color: '#fff', fontSize: '20px' }}>Ładowanie akcesoriów...</h2>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych akcesoriów damskich.</p>
            </div>
          )}
        </div>
      </div>
