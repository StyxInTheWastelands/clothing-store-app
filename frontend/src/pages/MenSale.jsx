...
function MenSale() {
  usePageMeta('Wyprzedaż męska', 'Produkty męskie w obniżonych cenach, do -50%.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      .then((res) => res.json())
      .then((data) => {
        const menSaleProducts = data.filter((product) => {
          const genderValue = product.gender ? product.gender.toLowerCase() : '';
          return (genderValue === 'men' || genderValue === 'męski') && product.is_sale === true;
        });
        setProducts(menSaleProducts);
      })
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '40px' }}>
          <div style={genderToggleWrapStyle}>
            <button onClick={() => navigate('/women-wyprzedaz')} style={genderPillStyle(false)}>Damski look</button>
            <button style={genderPillStyle(true)}>Męski look</button>
          </div>
        </div>
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie brak produktów w wyprzedaży dla mężczyzn.</p>
            </div>
          )}
        </div>
      </div>
