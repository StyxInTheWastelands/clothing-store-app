...
function WomenPants() {
  usePageMeta('Spodnie damskie', 'Spodnie z kolekcji damskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      .then((res) => res.json())
      .then((data) => {
        // Zabezpieczamy się na wypadek, gdyby data nie było tablicą
        const allProducts = Array.isArray(data) ? data : (data.products || []);

        // Filtrowanie: płeć 'women' oraz kategoria 2 (spodnie)
        const filteredProducts = allProducts.filter(product => {
          ...
          return isWomen && isPantsCategory;
        });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania spodni damskich:", err))
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych spodni damskich.</p>
            </div>
          )}
        </div>
      </div>
