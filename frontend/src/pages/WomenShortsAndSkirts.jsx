...
function WomenShortsAndSkirts() {
  usePageMeta('Spodenki i spódnice damskie', 'Spodenki i spódnice z kolekcji damskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      ...
      .then((data) => {
        ...
        // Filtrowanie: płeć 'women' oraz category_id === 3 (spodenki) LUB category_id === 7 (spódnice)
        const filteredProducts = allProducts.filter(product => {
          if (!product.gender) return false;
          const genderValue = product.gender.toLowerCase().trim();
          const isWomen = genderValue === 'women' || genderValue === 'kobieta';

          const categoryIdField = product.category_id !== undefined ? product.category_id : product.categoryId;
          const targetId = Number(categoryIdField);
          const isShortsOrSkirts = targetId === 3 || targetId === 7;

          return isWomen && isShortsOrSkirts;
        });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania spodenek i spódnic:", err))
  ...
  if (loading) {
    return (
      <div style={{ ...backgroundContainerStyle, justifyContent: 'center', padding: isMobile ? '0' : '40px 0' }}>
        <h2 style={{ color: '#fff', fontSize: '20px' }}>Ładowanie produktów...</h2>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych spodenek ani spódnic.</p>
            </div>
          )}
        </div>
      </div>
...
export default WomenShortsAndSkirts;