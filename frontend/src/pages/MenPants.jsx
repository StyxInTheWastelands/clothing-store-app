...
function MenPants() {
  usePageMeta('Spodnie męskie', 'Spodnie z kolekcji męskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      ...
      .then((data) => {
        ...
        // Filtrowanie: płeć 'men' / 'męski' oraz kategoria 2 (spodnie)
            const filteredProducts = allProducts.filter(product => {
            if (!product.gender) return false;

            const genderValue = product.gender.toLowerCase().trim();

            // Bezpieczna weryfikacja: sprawdzamy różne warianty zapisu płci męskiej
            const isMen =
                genderValue.startsWith('men') || 
                genderValue.startsWith('męs') || 
                genderValue.startsWith('mes') || 
                genderValue.includes('męż') ||
                genderValue.startsWith('m'); // najbardziej radykalny, ale skuteczny wariant, jeśli w bazie są tylko dwie płcie (m i w)

            const categoryIdField = product.category_id !== undefined ? product.category_id : product.categoryId;
            const isPantsCategory = Number(categoryIdField) === 2;

            return isMen && isPantsCategory;
            });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania spodni męskich:", err))
  ...
  if (loading) {
    return (
      <div style={{ ...backgroundContainerStyle, justifyContent: 'center' }}>
        <h2 style={{ color: '#fff', fontSize: '20px' }}>Ładowanie spodni...</h2>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych spodni męskich.</p>
            </div>
          )}
        </div>
      </div>
