...
function WomenJackets() {
  usePageMeta('Kurtki damskie', 'Kurtki z kolekcji damskiej Urban Stitch.');
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/products`)
      ...
      .then((data) => {
        ...
        // Filtrowanie: płeć żeńska + category_id === 4 (kurtki)
        const filteredProducts = allProducts.filter(product => {
          ...
          const isJacketCategory = Number(categoryIdField) === 4;

          return isWomen && isJacketCategory;
        });

        setProducts(filteredProducts);
      })
      .catch((err) => console.error("Błąd ładowania kurtek damskich:", err))
  ...
  if (loading) {
    return (
      <div style={{ ...backgroundContainerStyle, justifyContent: 'center' }}>
        <h2 style={{ color: '#fff', fontSize: '20px' }}>Ładowanie kurtek...</h2>
      </div>
    );
  }
        ...
        <div style={{
          ...
          {products.length > 0 ? (
            ...
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '40px' }}>
              <p style={{ color: '#6b7280' }}>Obecnie nie ma dostępnych kurtek damskich.</p>
            </div>
          )}
        </div>
      </div>
